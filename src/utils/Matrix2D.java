...
public class Matrix2D {
	...
	public static float stDevValue(float[][] base){
		if(base==null){ return Float.POSITIVE_INFINITY; }
		float[] matrix = new float[base.length*base[0].length];
		float average = 0;
		int entries = 0;
		for(int x=0;x<base.length;x++){
			for(int y=0;y<base[0].length;y++){
				matrix[x*base[0].length+y]=base[x][y];
				average += base[x][y];
				entries++;
			}
		}
		// calculate the mean
		average = average/(float)(entries);
		// calculate the sum of squares
		float sum = 0;
		int n = 0;
		for (int i=0;i<matrix.length;i++) {
			if(Double.isNaN(matrix[i]) || Double.isInfinite(matrix[i])){ continue; }
			final float v=matrix[i]-average;
			sum+=v*v;
			n++;
		}
	...
	public static double stDevValue(double[][] base){
		if(base==null){ return Float.POSITIVE_INFINITY; }
		double[] matrix = new double[base.length*base[0].length];
		double average = 0;
		int entries = 0;
		for(int x=0;x<base.length;x++){
			for(int y=0;y<base[0].length;y++){
				matrix[x*base.length+y]=base[x][y];
				average += base[x][y];
				entries++;
			}
		}
		...
		for (int i=0;i<matrix.length;i++) {
			if(Double.isNaN(matrix[i]) || Double.isInfinite(matrix[i])){ continue; }
			final double v=matrix[i]-average;
			sum+=v*v;
			n++;
		}
		if (n<2){ return Float.NaN; }
	...
	*/ 
	...
	public static float[][] makeFloat(double[][] base){
		if(base==null){ return null; }
		float[][] result = new float[base.length][base[0].length];
		for(int x=0;x<base.length;x++){
			for(int y=0;y<base[0].length;y++){
				result[x][y]=(float)base[x][y];
			}
		}
		return result;
	}
	...
	public static boolean[][] scaleUp(boolean[][] base, int zoomFactor){
		if(base==null){ return null; }
		boolean[][] result = new boolean[zoomFactor*base.length][zoomFactor*base[0].length];
		for(int x=0;x<base.length;x++){
			for(int y=0;y<base[0].length;y++){
				for(int i=0;i<zoomFactor;i++){
					for(int j=0;j<zoomFactor;j++){
						result[x*zoomFactor+i][y*zoomFactor+j]=base[x][y];
					}
				}
			}
		}
