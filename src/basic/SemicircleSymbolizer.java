...
public class SemicircleSymbolizer {
	int sizeX = 81;
	int sizeY = 81;
	...
	public PImage produceSymbol(){
		...
		while(Matrix2D.count(matrix)<sizeX*sizeY/5 || Matrix2D.count(matrix)>sizeX*sizeY*4/5){
			...
			int radius = (int)(0.5*Math.min(matrix.length,matrix[0].length))-edgeX-circleWidth-circlePadding;
			for(int x=0;x<matrix.length;x++){
				for(int y=0;y<matrix[0].length;y++){
					if(y<matrix[0].length/2){
						...
						if(dist>radius){
							matrix[x][y]=0.f;
							if(dist>radius+circlePadding && dist<=radius+circlePadding+circleWidth){
								matrix[x][y]=1.f;
							}
						}
					} else {
						if(x>=matrix.length-edgeX-circleWidth-circlePadding){
							matrix[x][y]=0.f;
						}
						if(x<=edgeX+circleWidth+circlePadding){
							matrix[x][y]=0.f;
						}
						if(y>=matrix[0].length-edgeX-circleWidth-circlePadding){
							matrix[x][y]=0.f;
						}
						if( (x>edgeX && x<=edgeX+circleWidth) || 
							(x<matrix.length-edgeX && x>=matrix.length-edgeX-circleWidth)){
							matrix[x][y]=1.f;
						}
						if(y>=matrix[0].length-edgeX){ 
							matrix[x][y]=0.f;
						}
						if( (x>edgeX && x<matrix.length-edgeX) && 
							(y<matrix[0].length-edgeX && y>=matrix[0].length-edgeX-circleWidth)){ 
							matrix[x][y]=1.f;
						}
					}
				}
	...
	public static void main(String[] args) {
		SemicircleSymbolizer sz = new SemicircleSymbolizer();
		for(int i=0;i<200;i++){
